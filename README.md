...
- **Comprehensive Analysis Suite**: Includes powerful, research-focused tools for automatically discovering, evaluating, and visualizing the performance of hundreds of model runs.
- **Parameterized by Design**: The entire workflow is built around the concept of treating physical parameters as inputs to the neural network, enabling the model to learn entire solution families.
...
- **PyTree Utilities**: Functions for mapping value ranges, slicing, batching, and consistency checks.
- **Serialization**: Helpers for saving/loading Equinox models, generic PyTrees, and HDF5 datasets.
- **Sampling & Reproducibility**: Implements stratified sampling and JAX PRNG key serialization.

#### `model.py`: The Neural Network Architecture
Defines the core `PINN` class, which is the neural network itself. It is built as an `equinox.Module`, integrating seamlessly with JAX's functional paradigm.
- **Configurable MLP**: Easily set network width, depth, and activation function.
- **Integrated Metadata**: Stores input/output mappings and coordinate spans directly in the model.
- **Versatile Evaluation**: Provides a low-level `__call__`, a `vmap`-based `eval` for batching, and a user-friendly `predict` API using named dictionaries.
- **Precision Control**: Supports both `float32` and `float64` parameters.
...
#### `residual.py`: Defining the Physics and Loss
Defines the physics of the problem by translating PDEs, ICs, and BCs into trainable residuals. **This is the main module to customize for new PDEs.**
- **Highly Customizable**: Designed to be adapted by the user for new PDEs and loss formulations.
- **Clear Structure**: Separates logic for IC, BC, PDE, and supervised data residuals.
- **Built-in Weighting**: Provides a standard, memory-efficient implementation of NTK-based loss weighting.
...
- **Batch Evaluation**: A memory-efficient, JIT-compiled evaluator to compute L1, L2, and Linf errors against reference data.
...
-   **`pde_dimless.py`**: A high-fidelity numerical solver for the ACCH system, built on `diffrax`. Its primary role is to generate ground-truth data for training and validation.
